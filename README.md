...
The data shown in the map are from [Cuebiq](https://cuebiq.com/) and represent the total number of *stops* in full years spanning from April 1, 2019 to March 31, 2020, April 1, 2023 to March 31, 2024, April 1, 2024 to March 31, 2025. These stops, which are measured using mobile devices like cell phones, "represent the points at which devices spend time, and are built according to the spatial and temporal proximity of individual device pings"

Each map in the Urban Activity Atlas shows a gradient of activity levels, i.e., the total number of stops, for the selected metropolitan region. Dark blue areas experienced the least amount of activity in the region between the specified years, and white areas experienced the most. Activity levels are standardized for each respective region, with values representing the total number of stops in the geohash divided by the total number of stops in the region overall. Therefore, activity levels should only be compared *within* regions, not *across* regions.

Percentage changes between the years show a difference in the normalized stop values. They serve to show which areas in the metro areas have increase or decreased in activity from one year to another. This comparison should again be served *within* regions, not *across* regions.

The data can be downloaded here:
[CSVs per metro folder](data/metro_region_geohash_stops_merged_6cols_csv)
